...
# Read data
################################################################################
d <- read.csv('credscoDB.csv', sep=";", stringsAsFactors = TRUE) # We want factors
...
plot_correlation(d,
                 type = "cont", 
                 geom_text_args = list(size=2))# Correlations
...
for (cV in columns_validate) {
  ...
  if (class(current_var) %in% c("integer", "numeric")) {
    ...
    gr_Hist    <- gghistogram(d, x  = cV,
                              add   = "mean",    rug  = TRUE,
                              color = "cluster", fill = "cluster")
    gr         <- ggarrange(gr_Boxplot, gr_Hist, 
  ...
  if (class(current_var) %in% c("factor", "character")) {
