...
The *Elections_92.RData*  file contains the results of a national sampling of potential voters. Specifically, it was a sample of 2198 nationwide voters in the United States on the act before the presidential elections in 1992.
...
```{r}
dfage <- with(elecc92, aggregate(x   = cbind(ypos = pres, yneg = 1 - pres),
                                 by  = list("age" = age), 
                                 FUN = sum))
head(dfage,4)
```
...
```{r}
m_age_agg <- glm(cbind(ypos,yneg) ~ age, data = dfage, 
...
```{r}
data.frame(dis = round(c(coef(m_age_dis),                       # coefficients
                         deviance = m_age_dis$deviance,         # deviance
                         df       = m_age_dis$df.residual),3),  # degrees of freedom
           agg = round(c(coef(m_age_agg), 
                         deviance = m_age_agg$deviance, 
                         df       = m_age_agg$df.residual),3))
```
